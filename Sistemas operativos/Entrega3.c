...
void VentanaRegistros(WINDOW *IDventanaRegistros, struct PCB *pcb)
{
    ...
    mvwprintw(IDventanaRegistros, 2, 20, "BX: %d        ", pcb->BX);
    mvwprintw(IDventanaRegistros, 4, 5, "CX: %d        ", pcb->CX);
    mvwprintw(IDventanaRegistros, 4, 20, "DX: %d        ", pcb->DX);
    mvwprintw(IDventanaRegistros, 6, 5, "PC: %d        ", pcb->PC);
    mvwprintw(IDventanaRegistros, 6, 20, "IR: %s  ", pcb->IR);
...
void ImprimirListos(WINDOW *IDventanaProcesos, struct PCB *PrimerNodo)
{
    ...
    if (NodoActual != NULL)
    {
        ...
        while (NodoActual != NULL)
        {
            ...
            mvwprintw(IDventanaProcesos, 11 + i, 2, "PID: %d, FileName: %s, AX: %d, BX: %d, CX: %d, DX: %d, PC: %d, IR: %s",
                      NodoActual->PID, NodoActual->fileName, NodoActual->AX, NodoActual->BX, NodoActual->CX, NodoActual->DX, NodoActual->PC, NodoActual->IR);
...
void ImprimirTerminados(WINDOW *IDventanaProcesos, struct PCB *PrimerNodo)
{
    ...
    if (NodoActual != NULL)
    {
        ...
        while (NodoActual != NULL)
        {
            mvwprintw(IDventanaProcesos, height / 2 + 1 + i, 2, "PID: %d, FileName: %s, AX: %d, BX: %d, CX: %d, DX: %d, PC: %d, IR: %s",
                      NodoActual->PID, NodoActual->fileName, NodoActual->AX, NodoActual->BX, NodoActual->CX, NodoActual->DX, NodoActual->PC, NodoActual->IR);
            NodoActual = NodoActual->sig;
            i++;
        }
...
// Declarar IDventanaProcesos como una variable global
WINDOW* IDventanaProcesos = NULL;
struct ProgramaCargado* ProgramasCargados = NULL;

// Declarar una estructura para almacenar los programas cargados
struct ProgramaCargado {
  char nombreArchivo[200];
  struct ProgramaCargado* siguiente;
};

// Declarar una variable global para contar los programas cargados
int contadorProgramasCargados = 0;

// Función para agregar un programa cargado a la lista
void agregarProgramaCargado(struct ProgramaCargado** lista, char* nombreArchivo) {
  // Verificar si el programa ya está en la lista
  struct ProgramaCargado* actual = *lista;
  while (actual != NULL) {
    if (strcmp(actual->nombreArchivo, nombreArchivo) == 0) {
      return; // El programa ya está en la lista, no se agrega nuevamente
    }
    actual = actual->siguiente;
  }

  // Crear un nuevo nodo para el programa cargado
  struct ProgramaCargado* nuevoPrograma = (struct ProgramaCargado*)malloc(sizeof(struct ProgramaCargado));
  strcpy(nuevoPrograma->nombreArchivo, nombreArchivo);
  nuevoPrograma->siguiente = NULL;
  ...
  if (*lista == NULL) {
    *lista = nuevoPrograma;
    ...
    actual->siguiente = nuevoPrograma;
  }

  // Incrementar el contador de programas cargados
  contadorProgramasCargados++;

  // Mostrar el número de programas cargados en la ventana de procesos
  mvwprintw(IDventanaProcesos, 1, 5, "Programas cargados: %d", contadorProgramasCargados);
}

// Función para eliminar un programa cargado de la lista
void eliminarProgramaCargado(struct ProgramaCargado** lista, char* nombreArchivo) {
  // Verificar si el programa está en la lista
  struct ProgramaCargado* actual = *lista;
  struct ProgramaCargado* anterior = NULL;
  while (actual != NULL) {
    if (strcmp(actual->nombreArchivo, nombreArchivo) == 0) {
      ...
      // Decrementar el contador de programas cargados
      contadorProgramasCargados--;
...
// Función para obtener la cantidad de programas cargados
int obtenerCantidadProgramasCargados() {
  return contadorProgramasCargados;
}
...
int main(void)
{
    ...
    IDventanaProcesos = initscr(); // Inicializar la ventana con ncurses
    ...
    while (EstadoLineaComandos != 666)
    {
      ...
      agregarProgramaCargado(&ProgramasCargados, ArchivoCargadoValido);
      eliminarProgramaCargado(&ProgramasCargados, ArchivoCargadoValido);
